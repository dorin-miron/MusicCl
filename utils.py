...
DATASET_DIR = config["GENRE_DIR"]
TEST_DIR = config["TEST_DIR"]
GENRE_LIST = config["GENRE_LIST"]

if DATASET_DIR is None or DATASET_DIR is "":
...
def convert_dataset_to_wav():
    ...
    print "Start convert Dataset to WAV format"
...
def plot_confusion_matrix(cm, genre_list, name, title):
    ...
    ax.set_xticklabels(genre_list, rotation=45)
    ...
    pylab.colorbar()
    pylab.grid(False)
    pylab.xlabel('Genul prezis', fontsize=20)
    pylab.ylabel('Genul real', fontsize=20)
    pylab.grid(False)
